...
We considered two different simulations to gain additional insight into how correlation in space use influences local FOI. We use these simulations to explore the relative contribution of correlation to overall FOI, and how it is influenced by parasite decay, the difference in scale of movement with respect to the transmission distance, as well as data availability.  %First we simulate individuals that follow each other across a gridded landscape according to a set hierarchy. We then make a more complex movement simulation to mimic the data generated by empirical telemetry methods (e.g. GPS collars). With these simulations we explore 
