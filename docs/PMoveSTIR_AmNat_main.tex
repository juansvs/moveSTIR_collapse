...
As derived in \cite{Wilber2022} (and re-derived in Appendix X), MoveSTIR defines the pairwise FOI host $j$ exerts on host $i$ in location $x$ at time $t$ as \citep{Wilber2022}
\begin{equation}
...
\end{equation}
The parameter $\lambda$ is the per capita pathogen deposition rate. We assume that $\lambda$ is constant and does not depend on time since infection. The variable $\delta_{x_j(u)}(x)$ is an indicator function that is one if host $j$ is in location $x$ at time $u$ and zero otherwise, $\delta_{I_j(u)}(I)$ is an indicator function that is one if host $j$ is infected at time $u$ and zero otherwise.  The function $\Theta(t-u)$ is a pathogen survival function that gives the probability that pathogens deposited at time $u < t$ are still transmissible at time $t$ (see Appendix X for a full derivation). 

The parameter $\beta'$ is the rate at which host $i$ acquires pathogen within location $x$ and can be re-written as $\tilde{\beta} / A_x$, where $\tilde{\beta}$ can be considered a ``search efficiency'' term, with units area/time (e.g., $m^2 / day$), and $A_x$ gives the area of location $x$ where transmission can occur (e.g., 100 $m^2$). In equation \ref{eq:original_foi}, we assume that the likelihood of contact is uniform within location $x$ -- in other words, once two hosts are both within the area $x$, it no longer matters for contact how close they are to each other within this area as the likelihood of contact and transmission is always the same.  This is equivalent to a top-hat encounter function \citep{Gurarie2013,Wilber2022} where contact likelihood is uniform within a circle of radius $r$ around a focal individual. Here, however, area $x$ is not necessarily circular. Importantly, the location $x$ and its area $A_x$ is a biological property of the pathogen and is separate from the movement ecology of the host or the discretization of movement data. Area $x$ effectively defines how close individuals need to be before transmission can occur. If $x$ is circular, then two hosts need to be at least within two times the radius of $x$ for transmission to occur.  If $x$ is a square area (as we will use later in this study), then hosts generally need to be within the length of the side of the square for transmission to occur (but could be farther apart if they are along the diagonal of the square).  

It is also useful to consider the limitations of two of our assumptions underlying equation \ref{eq:original_foi} -- density-dependent transmission and no substantial pathogen depletion in the environment following infection. The parameter $\lambda$ is a per capita shedding rate and is not scaled by area. Over a unit of time, infected hosts will shed a finite amount of pathogen into the location $x$ that they are occupying.  Equation \ref{eq:original_foi} assumes that this pathogen instantaneously and uniformly fills the location $x$ such that there is a density of pathogen $\lambda / A_x$ in $x$. Another host subsequently searches location $x$ with some search efficiency, acquiring pathogen are rate $\tilde{\beta}$. But what happens as $A_x$ approaches zero?  The deposited pathogen becomes infinitely dense and, because we assume there is no pathogen depletion upon acquisition (Appendix X), FOI goes to infinity.  While this limit is logically consistent with density-dependent transmission and no pathogen depletion, it is biologically nonsensical because a discrete host cannot occupy, shed, and acquire pathogen in an infinitely small area. Thus, there is a biological lower bound on $A_x$ that ensures that at least one host can fit into $x$.  While you can mathematically shrink $A_x$ below this limit, it does not make any biological sense to do this and for what follows we will always consider that $A_x$ at minimum can encompass the host and is often larger than the host (e.g., consider the 2 meter social distancing guidelines during the COVID-19 pandemic based on the spread of pathogen droplets).
...
We now derive the PMoveSTIR model from equation \ref{eq:original_foi}. We first simplify equation \ref{eq:original_foi} by assuming that the depositing host is always infected, and shedding pathogen infectious stages at a constant rate. This assumption is equivalent to building a contact network and also represents the structural form of FOI needed to compute pathogen invasion thresholds \citep{Wilber2022} [Reviewer thinks this is unclear...update].  
...
\begin{equation}
    E[h_{i \leftarrow j}(t, x)] := h^*_{i \leftarrow j}(t, x) = \int_{-\infty}^{t} \beta' \lambda E[\delta'_{x_i(t)}(x) \delta'_{x_j(u)}(x)] \Theta(t - u) du.
    \label{eq:expected_foi}
\end{equation}
Interpreting this expectation, we are asking: if we simulated some movement process thousands of times, what is the probability that host $i$ is in location $x$ at time $t$, and host $j$ was in $x$ at a previous time $u$?  While this is a fairly simple mathematical addition to the MoveSTIR equation \ref{eq:original_foi}, it is a significant conceptual change as it provides the mathematical structure necessary to link directly with UDs and partition the contributions of space use and non-independent movements as we describe next.
...
This general PMoveSTIR formulation accounts for heterogeneity in space and time, but the model can consider other scenarios, such as uniform space use, time-varying UDs, or UDs stationary in time (Fig. \ref{fig:square}, Appendix B, C).  Note that \cite{Wilber2022} in Appendix X did consider a special case of PMoveSTIR looking at home-range overlap, but they did not link this equation to UDs and did not consider correlation in movement. [Transition]

For example, when movement is statistically stationary in time, and pathogen decays exponentially in the environment at rate $\nu$, equation \ref{eq:foi_cov} becomes (derivation in Appendix B)

\begin{equation}
    \begin{aligned}
    h^*_{i \leftarrow j}(x) = \beta' \lambda [ \underbrace{p_i(x)p_j(x) \frac{1}{\nu}}_{\substack{\text{FOI contribution from} \\ \text{shared space use}}} + \sigma_i(x) \sigma_j(x) \underbrace{\int_{0}^{\infty} Cor(\delta_{i \text{ in } x}, \delta_{j \text{ in } x} | s) e^{-\nu s} ds}_{\substack{\text{FOI contribution from} \\ \text{correlated movement}}}.
    \end{aligned}
    \label{eq:stationary_cor}
\end{equation}
where $\sigma_i(x) = \sqrt{p_i(x)(1 - p_i(x))}$ is the standard deviation in probability of host $i$ using location $x$, and $\delta_{i \text{ in } x}$, $\delta_{j \text{ in } x}$ are 0/1 random variables that specify whether or not $i$, $j$ are in location $x$. The term $Cor(\delta_{i \text{ in } x}, \delta_{j \text{ in } x} | s)$ gives the correlation between occupancy vectors at a time lag of $s$ [Update in the Appendix and talk about $s$...THIS IS VAGUE...MAKE THIS MORE EXPLICIT...check on time series literature to get this notation right on the time lag and random variable...]. 

Putting equation \ref{eq:stationary_cor} in context, all previous studies we are aware of that infer contact and transmission risk from movement data and UDs have assumed $h^*_{i \leftarrow j}(x) \propto p_i(x)p_j(x)$ \citep[e.g.,][]{Noonan2021} [Janine, lizards, JAE, others! ]. However, equation \ref{eq:stationary_cor} makes it explicit that the expected FOI between two individuals is a result of 1) shared space use $p_i(x)p_j(x)$ and 2) correlation in host movements across different time lags. Using equation \ref{eq:stationary_cor} moving forward, we ask: how much does the term $\sigma_i(x) \sigma_j(x) \int_{0}^{\infty} Cor(\delta_{i \text{ in } x}, \delta_{j \text{ in } x} | s) e^{-\nu s} ds$ actually matter for FOI under different movement strategies and epidemiological scenarios? We consider two analytical analyses, two simulation analyses, and one empirical analysis to address this question, as described next.

\subsection*{Analytical analysis 1: The effect of correlated movements on FOI for random movement and direct contact}

Consider the situation where movement is statistically stationary and hosts use space uniformly (equation S11...expand here). 
For illustrative purposes, consider two hosts moving across some landscape with total area $A_{tot}$. $A_{tot}$ is a property of the hosts, indicating their range of movement. The area of transmission is $A_x$ is a property of the pathogen and determines how close two hosts have to be in order for transmission to occurs. For example, location $x$ with area $A_x$ might be a circle with radius $r$, indicating that hosts must be within at least $2r$ of each other for transmission to occur. 

Further, we assume that hosts spend on average $\eta$ time units within the area $A_x$ of location $x$ before moving elsewhere.  The residence time $\eta$ will of course scale with $A_x$ as hosts moving at a constant rate across the landscape will spend less time in smaller areas.  We do not make this scaling explicit it will cancel out it our subsequent analysis. We also assume that the pathogen survival function $\Theta(s)$ is a step function with a survival probability of one when lag $s \leq \pi \eta$ and zero when $s > \pi \eta$. 
The term $\pi \eta$ gives the time the pathogen survives in the environment as a function of host residence time, where $\pi$ ranges from near zero for directly transmitted pathogens to some arbitrarily large number for pathogens with long environmental persistence.  With these assumptions, we can rewrite equation S11 as 
\begin{equation}
    \begin{aligned}
        h^*_{i \leftarrow j}(x) = \beta' \lambda \left[\frac{A_x}{A_{tot}}\frac{A_x}{A_{tot}} \pi \eta + \frac{A_x}{A_{tot}}(1 - \frac{A_x}{A_{tot}}) \int_{0}^{\pi \eta} Cor(\delta_{i \text{ in } A_x}, \delta_{j \text{ in } A_x} | s) ds\right],
    \end{aligned}
...
The correlation term $\rho = Cor(\delta_{i \text{ in } A_x}, \delta_{j \text{ in } A_x} | s=0)$ will be exactly $\rho$ when lag $s = 0$ and near $\rho$ when lag $s$ is near zero. When pathogens are strictly directly transmitted, $\pi \eta << \eta$ then we can reasonably approximate $Cor(\delta_{i \text{ in } A_x}, \delta_{j \text{ in } A_x} | s) = \rho$ for s from 0 to $\pi \eta$.  We can then write equation \ref{eq:uniform_stationary2} as 

\begin{equation}
    \begin{aligned}
        h^*_{i \leftarrow j}(x) = \beta' \lambda \pi \eta \left[\underbrace{\frac{A_x}{A_{tot}}\frac{A_x}{A_{tot}}}_{\substack{\text{Contribution due} \\  \text{to habitat overlap}}} + \underbrace{\frac{A_x}{A_{tot}}(1 - \frac{A_x}{A_{tot}}) \rho}_{\substack{\text{Contribution due} \\ \text{to correlated movement}}} \right].
    \end{aligned}
    \label{eq:uniform_direct}
\end{equation}
The relative contribution of correlation in movement with respect to the contribution due to habitat overlap is simply $[(1 - (A_x / A_{tot}))\rho] / (A_x / A_{tot})=(A_{tot}/A_x-1)\rho$. Thus, PMoveSTIR allows us to put intuitive bounds on the importance of correlated movements for direct transmission risk. 

Before exploring this relationship, it is important to note that the value $\rho$ depends on $A_x$ and $A_{tot}$. However, the exact way that $\rho$ scales with $A_x$ and $A_{tot}$ strongly depends on the movement strategies of the hosts. To illustrate an example of this relationship, we implement a simple follow-the-leader model that allows us to analytically show how $\rho$ relates to $A_{x}$, $A_{tot}$, and social decisions that hosts make during movement. The important point here is that while $\rho$ is not fixed across scales, we can reasonably look at how independently varying values of $A_x / A_{tot}$ and $\rho$ affect the relative contributions of correlated movement because $\rho$ is also driven by biological parameters (e.g., mechanisms of attraction) that could be independent of $A_x$ and $A_{tot}$.  While different host-parasite systems will fall in different regions of this $\rho-(A_x / A_{tot})$ space, movement 
...
\subsection*{Simulation analysis 1: }
